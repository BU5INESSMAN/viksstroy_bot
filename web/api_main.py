...
def strip_html(text):
    """Удаляет HTML теги из строки для корректного отображения в MAX"""
    if not text: return ""
    clean = re.compile('<.*?>')
...
async def notify_users(target_roles: list, text: str, url_path: str = "dashboard", extra_tg_ids: list = None):
    ...
    max_group_id = os.getenv("MAX_GROUP_CHAT_ID")
    ...
    # Формируем чистый текст (без тегов) и ссылку для MAX
    ...
    async with aiohttp.ClientSession() as session:
        for cid_raw in tg_chat_ids:
            ...
            if cid_int < 0:
                # Отправка личного уведомления пользователю MAX
                if max_bot_token:
                    try:
                        await session.post(
                            "https://platform-api.max.ru/messages",
                ...
                # Отправка личного уведомления пользователю Telegram
                if bot_token:
                    try:
                        await session.post(
...
async def execute_app_publish(app_dict):
    ...
    # Сохраняем картинку локально, чтобы MAX мог получить её по публичному URL
...
# НОВЫЙ ЭНДПОИНТ: ТЕСТОВАЯ ОТПРАВКА УВЕДОМЛЕНИЙ
@app.post("/api/system/test_notification")
async def test_notification(tg_id: int = Form(...)):
    ...
    # 2. Уведомление модераторам о создании
    await notify_users(["moderator"], f"📝 <b>Тестовая заявка:</b>\n👷‍♂️ Прораб: {fio}\n📍 Объект: Проверка уведомлений",
                       "review", [real_tg_id])

    # 3. Публикация фейкового наряда в общую группу (MAX + TG) с генерацией картинки
    fake_app = {
        'id': 9999,
        ...
        'foreman_name': fio,
        'team_name': 'Тестовая бригада MAX',
        'selected_members': '',
        'equipment_data': '[]',
...
async def execute_app_publish(app_dict):
    ...
    # Если отправлено хотя бы в один мессенджер, помечаем как успешное
    if published_tg or published_max:
        try:
            await db.conn.execute("UPDATE applications SET status = 'published' WHERE id = ?", (app_id,))
            if eq_data_str:
