...
def download_font(url, filename):
    if not os.path.exists(filename) or os.path.getsize(filename) < 10000:
        ...
        except Exception as e: print(f"Font download error: {e}")
...
def create_app_image(date_str, address, foreman, team_name, equip_list, comment_str=""):
    ...
    if not logo_drawn:
        ...
        draw.text(((img_w - (bbox[2]-bbox[0]))//2, 40 + (header_h - (bbox[3]-bbox[1]))//2), "ВИКС РАСПИСАНИЕ", fill=(255, 255, 255), font=font_header)
...
async def notify_users(target_roles: list, text: str, url_path: str = "dashboard", extra_tg_ids: list = None):
    ...
    if roles_to_fetch:
        ...
        try:
            async with db.conn.execute(f"SELECT user_id FROM users WHERE role IN ({pl}) AND is_blacklisted = 0", roles_to_fetch) as cur:
    ...
    markup = {"inline_keyboard": [[{"text": "📱 Открыть платформу", "web_app": {"url": f"https://islandvpn.sbs/{url_path}"}}]]}
    
    async with aiohttp.ClientSession() as session:
        for cid in chat_ids:
            try: await session.post(f"https://api.telegram.org/bot{bot_token}/sendMessage", json={"chat_id": cid, "text": text, "parse_mode": "HTML", "reply_markup": markup})
            except: pass
...
async def background_scheduler():
    ...
    while True:
        try:
            ...
            if auto_pub_time and current_time_str == auto_pub_time and last_auto_publish_date != current_date_str:
                ...
                if apps:
                    ...
                    await notify_users(["moderator", "report_group"], f"⚠️ <b>Авто-публикация</b>\nНа сегодня ({current_date_str}) нет одобренных заявок для автоматической публикации.", "review")
        ...
        except Exception as e: print(f"Bg Scheduler error: {e}")
...
async def startup():
    ...
    columns_to_add = [
        ("applications", "foreman_id", "INTEGER"), ("applications", "foreman_name", "TEXT"), ("applications", "equip_id", "INTEGER DEFAULT 0"),
...
async def telegram_auth(data: dict):
    try:
        ...
        if user:
            ...
            return {"status": "ok", "role": user_dict['role'], "fio": user_dict['fio'], "tg_id": tg_id, "avatar_url": user_dict.get('avatar_url', photo_url)}
        return {"status": "needs_password", "tg_id": tg_id, "first_name": data.get('first_name', ''), "last_name": data.get('last_name', ''), "photo_url": photo_url}
...
async def api_get_users():
    ...
    return [{"user_id": dict(u)['user_id'], "fio": dict(u)['fio'], "role": dict(u)['role'], "is_blacklisted": dict(u)['is_blacklisted'], "avatar_url": dict(u).get('avatar_url', '')} for u in users]
...
async def api_update_avatar(target_id: int, avatar_url: str = Form(""), avatar_base64: str = Form(""), tg_id: int = Form(0)):
...
async def api_update_profile(target_id: int, tg_id: int = Form(...), fio: str = Form(...), role: str = Form(...), team_id: int = Form(0), position: str = Form("")):
    ...
    if not admin or admin['role'] not in ['superadmin', 'boss', 'moderator']: raise HTTPException(status_code=403, detail="Нет прав")
    ...
    elif team_id > 0: await db.conn.execute("INSERT INTO team_members (team_id, fio, position, tg_id) VALUES (?, ?, ?, ?)", (team_id, fio, position, target_id))
    await db.conn.commit()
    return {"status": "ok"}
...
async def api_delete_user(target_id: int, tg_id: int = Form(...)):
    admin = await db.get_user(tg_id)
    if not admin or dict(admin).get('role') not in ['superadmin', 'boss', 'moderator']: raise HTTPException(403, "Нет прав")
    try:
...
async def api_get_invite_info(invite_code: str):
    ...
    return {"team_name": team['name'], "unclaimed_workers": [{"id": w['id'], "fio": w['fio'], "position": w['position']} for w in await db.get_unclaimed_workers(team['id'])]}
...
async def api_join_team(invite_code: str = Form(...), worker_id: int = Form(...), tg_id: int = Form(...)):
    ...
    await db.conn.commit()
    await notify_users(["report_group"], f"🔗 <b>Привязка аккаунта</b>\nРабочий {fio} привязал Telegram к бригаде «{team['name']}».", "teams")
    return {"status": "ok"}
...
async def execute_app_publish(app_dict, bot_token, group_id):
    ...
    if selected_list:
        ...
        async with db.conn.execute(f"SELECT fio, position, tg_id FROM team_members WHERE id IN ({pl})", selected_list) as cur:
