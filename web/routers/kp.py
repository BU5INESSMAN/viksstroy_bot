...
async def download_kp_catalog(current_user=Depends(_require_office)):
    """Отдает последний загруженный файл прайса. Office+ (moderator/boss/superadmin).

    Audit: logs every export with action='catalog_exported' so we have
    traceability for the loosened access threshold.
    """
...
async def upload_kp_catalog(file: UploadFile = File(...), current_user=Depends(_require_office)):
    ...
    SECURITY: Loosened from superadmin to office+ (moderator/boss/superadmin)
    on 2026-05-18. Compensating control: every import is audit-logged with
    file name, row count, and the role of the uploader.
    """
