...
async def _fetch_schedule_sections(target_date: str) -> list:
    ...
    # Driver name now comes from the user with default_equipment_id = e.id
    # (the new driver-as-user model). The legacy equipment.driver_fio is
    # kept as a fallback for rows the migration didn't touch.
    async with db.conn.execute(
