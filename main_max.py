...
async def message_callback(event: MessageCallback):
    ...
    if payload.startswith("login_approve|") or payload.startswith("login_deny|"):
        action, request_id = payload.split("|", 1)
        if not re.fullmatch(r"[a-f0-9]{32}", request_id):
            return await send_max_msg(event, "❌ Некорректный запрос входа.")
        now = time.time()
        new_status = "approved" if action == "login_approve" else "denied"
        result = await db.conn.execute(
            "UPDATE max_login_requests SET status = ?, approved_at = ? "
            "WHERE request_id = ? AND user_id = ? AND status = 'pending' AND expires >= ?",
            (new_status, now, request_id, real_tg_id, now),
        )
        await db.conn.commit()
        if result.rowcount != 1:
            return await send_max_msg(event, "⚠️ Запрос уже использован или истёк.")
        if new_status == "approved":
            return await send_max_msg(event, "✅ Вход подтверждён. Вернитесь в ВиКС.")
        return await send_max_msg(event, "⛔ Вход отклонён. Сессия не создана.")
