...
`web/routers/auth.py` — Эндпоинты аутентификации. Основной беспарольный вход работает через WebAuthn/Passkey (`/api/auth/passkeys/*`): ключ создаётся внутри активной сессии, требует системную проверку пользователя и затем создаёт HttpOnly-сессию. Вход через `POST /api/auth/max-login/start` + `POST /api/auth/max-login/poll` доступен только для заранее привязанного браузера/PWA и подтверждается callback-кнопками в личном сообщении MAX. `POST /api/auth/code` с 6-значным кодом сохранён как резервный первичный вход. `POST /api/users/link_account` — привязка старой записи к основному MAX-профилю. `POST /api/users/unlink_platform` — отвязка старой записи. `POST /api/max/web_auth` — веб-аутентификация для MAX. `POST /api/max/auth`, `POST /api/max/register` — аутентификация и регистрация MAX с паролем. Telegram-аутентификация и её маршруты удалены в v2.15.0.
...
`web/routers/applications.py` — Ядро системы: управление заявками (нарядами). `POST /api/applications/check_availability` — проверка занятости бригад/техники. `POST /api/applications/create` — обычное создание или ретроспективная заявка строго за вчера с `is_backdated=true`; ретроспективная заявка сразу завершена, не публикуется и используется только для СМР. `POST /api/applications/{app_id}/update` — редактирование (статус waiting). `POST /api/applications/{app_id}/delete` — полное удаление с освобождением техники. `GET /api/applications/review` — заявки для модерации. `POST /api/applications/{app_id}/review` — смена статуса (approved/rejected/completed) с уведомлениями. `POST /api/applications/publish` — ручная публикация в группы. `GET /api/applications/active` — активные наряды текущего пользователя. `GET /api/applications/my` — завершённые наряды. `POST /api/applications/{app_id}/free_equipment` — освобождение техники водителем. `POST /api/applications/{app_id}/free_team` — освобождение бригады. `POST /api/applications/{app_id}/release_resources` — массовое освобождение выбранных бригад и техники самим прорабом с единым точным временем.
...
`database/apps_repo.py` — Миксин `AppsRepoMixin`. Методы: `save_application`, `get_application_details`, `update_app_status`, `get_pending_applications`, `get_user_applications`, `get_daily_report`, `get_object_history`, `get_app_members_with_tg`, `get_approved_apps_for_publish`, `mark_app_as_published`, `check_resource_availability` (проверка конфликтов бригад/техники с учётом отдельно освобождённых бригад и техники).

`application_resource_releases` хранит фактическое время освобождения каждой бригады или единицы техники. Эти отметки используются доступностью и статистикой, поэтому несколько последовательных объектов в один день считаются раздельно. `applications.is_backdated` и `backdated_created_at` фиксируют ретроспективную заявку и момент её создания; режим доступен только при первичном создании.
...
`database/kp_repo.py` — Миксин `KpRepoMixin`. Методы: `get_kp_dashboard_apps` (распределение по вкладкам), `get_app_kp_items`, `get_group_kp_items`, `submit_kp_report` (с условным одобрением для foreman+), `review_kp_report`, `update_kp_volumes_only`, `generate_mass_excel` (Excel через pandas/openpyxl). Для объединённого СМР каждая плановая работа возвращается вместе с исходной заявкой и объектом. Одинаковая позиция КП на разных объектах остаётся двумя независимыми строками и сохраняется обратно в соответствующие заявки.

Импорт справочника в `KpRepoMixin.import_kp_from_excel` имеет строгий контракт D/E/G/H: название работы, цена СМР, единица измерения и расценка ЗП соответственно. Остальные столбцы исходного Excel являются внешними расчётами и не читаются. Строка заголовков определяется семантически в первых 30 строках, а применение выполняется транзакционно с сохранением ID существующих позиций.
...
`database/hours_repo.py` — Миксин `HoursRepoMixin` (СМР wizard, шаг 1 «Часы»). Методы: `get_app_hours`, `save_app_hours`, `get_teams_for_app`, `get_user_team_ids` (область видимости бригадира). Хранит часы по участникам бригад в `application_hours`. В объединённом СМР для каждой исходной заявки и объекта создаётся отдельная секция бригады: один сотрудник может иметь разные часы и ЗП на разных объектах.

`smr_data.py` — единая модель чтения логического СМР. Каждая строка часов, основной или дополнительной работы получает ровно одну исходную заявку и один объект. Новые данные используют сохранённый источник напрямую; старые строки восстанавливаются по составу заявки и плану КП, а при неоднозначности безопасно остаются у физической заявки вместо объединения названий через слеш. Excel, хронология и массовая выгрузка используют ту же привязку.
