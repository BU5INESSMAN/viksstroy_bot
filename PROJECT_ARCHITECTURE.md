...
`smr_data.py` — единая модель чтения логического СМР. Помимо расчётов она восстанавливает объект и исходную заявку каждой строки объединённого отчёта: для сотрудников по выбранному составу заявки, для работ по плану КП объекта. Excel и массовая выгрузка используют эту привязку; неоднозначная историческая строка получает перечень всех подходящих объектов.
