...
const CHANGELOG = [
    {
        version: '2.6', date: '2026-05-19', title: 'Реструктуризация водителей', current: true,
        commits: 8,
        changes: [
            { type: 'feat', text: 'Водители — отдельные пользователи системы с личным кодом входа (users.invite_code)' },
            { type: 'feat', text: 'Назначение водителя на каждую единицу техники в заявке (раньше — только привязка к технике)' },
            { type: 'feat', text: 'Проверка занятости водителя при назначении: один водитель не может быть на двух машинах в пересекающееся время' },
            { type: 'feat', text: 'Водитель по умолчанию — теперь свойство техники; назначает офис на странице «Техника»' },
            { type: 'feat', text: 'Picker водителей с сортировкой: ⭐ по умолчанию → недавние → популярные → алфавит' },
            { type: 'feat', text: 'Hard-block конфликтов в picker: занятые водители показаны серым с ⚠ ЗАНЯТ и tooltip\'ом где они заняты' },
            { type: 'feat', text: 'Inline-форма «+ Новый водитель» в picker: ФИО + мультивыбор категорий, без модалок-в-модалках' },
            { type: 'feat', text: 'KP — каталог теперь доступен модераторам+, аудит каждой загрузки/выгрузки' },
            { type: 'feat', text: 'Эндпоинт GET /api/drivers/availability?date=… для проверки занятости на дату' },
            { type: 'feat', text: 'Эндпоинт PATCH /api/equipment/{id}/default-driver (require_office, аудит)' },
            { type: 'fix', text: 'Восстановлены ФИО для 11 водителей, у которых после ранее ломаной миграции остались placeholder\'ы «Пользователь -X»' },
            { type: 'fix', text: 'Починен сломанный массовый импорт техники: формат теперь `название | категория | госномер`, старый формат с ФИО отклоняется с понятной ошибкой' },
            { type: 'fix', text: 'Защита на сервере (defense-in-depth): backend отклонит назначение конфликтующего водителя даже если фронт пропустит' },
            { type: 'refactor', text: 'Аутентификация водителей переведена на личный users.invite_code вместо привязки к технике' },
            { type: 'refactor', text: 'Сессии активных водителей сброшены при выкатке — потребуется повторный вход по новому коду' },
            { type: 'refactor', text: 'Старые ссылки /equip-invite/{code} автоматически открывают сессию водителя «по умолчанию» этой техники' },
            { type: 'refactor', text: 'Карточка водителя на странице «Ресурсы» очищена: офис управляет defaults на стороне техники' },
            { type: 'refactor', text: 'Удалены поля ФИО из формы добавления техники: единичной и массовой' },
            { type: 'refactor', text: 'Legacy-колонки equipment.driver / driver_fio / tg_id / invite_code и users.default_equipment_id помечены DEPRECATED — будут удалены в v2.7+' },
        ],
    },
    {
        version: '2.5', date: '2026-04-22', title: 'СМР и управление бригадами',
        commits: 12,
        changes: [
            { type: 'feat', text: '3-шаговый мастер заполнения СМР (Часы → Работы → Просмотр)' },
            { type: 'feat', text: 'Учёт отработанных часов по бригадам и сотрудникам' },
            { type: 'feat', text: 'Роли в СМР: бригадир заполняет → прораб проверяет и одобряет' },
            { type: 'feat', text: 'Переключатель «Общие работы / По бригадам» с итогами' },
            { type: 'feat', text: 'Объединение заявок СМР для совместного заполнения' },
            { type: 'feat', text: 'Отмена объединения заявок' },
            { type: 'feat', text: 'Частичное использование бригад — свободные участники в другие заявки' },
            { type: 'feat', text: 'Жёлтая пометка «(част.)» для частично занятых бригад' },
            { type: 'feat', text: 'Предупреждение при выборе бригады без бригадира' },
            { type: 'feat', text: 'Отображение занятых участников с номером заявки в выборе бригады' },
            { type: 'feat', text: 'Группировка заявок по объектам и датам на странице СМР' },
            { type: 'feat', text: 'Excel-отчёт СМР: часы + работы + доп. работы, без ЗП' },
            { type: 'feat', text: 'Имя файла отчёта: Объект_Дата.xlsx' },
            { type: 'feat', text: 'Иконки Tabler Icons для техники и бригад' },
            { type: 'feat', text: 'Экран обновления системы при недоступности сервера' },
            { type: 'feat', text: 'Модератор может менять статус заявок' },
            { type: 'feat', text: 'Расширенное тестирование уведомлений в админке' },
            { type: 'feat', text: 'Все команды бота реализованы (TG + MAX)' },
            { type: 'fix', text: 'Заполнение СМР без ограничения по времени' },
            { type: 'fix', text: 'Push-уведомления: VAPID aud claim' },
            { type: 'fix', text: 'Парсер каталога КП: единицы из правильной колонки' },
            { type: 'fix', text: 'Онлайн-пользователи: трекинг через session cookie' },
            { type: 'fix', text: 'Время техники по умолчанию из настроек' },
            { type: 'fix', text: 'Полная очистка при истечении сессии' },
            { type: 'fix', text: 'Статистика объекта: данные из заполненных отчётов + доп. работы' },
            { type: 'fix', text: 'Хронология выполнения в статистике объекта' },
            { type: 'refactor', text: 'Кнопки действий перенесены из просмотра заявки в проверку' },
            { type: 'refactor', text: 'Компактная техника в канбане без водителей' },
            { type: 'refactor', text: 'Просмотр заявки — только чтение' },
        ],
    },
    {
        version: '2.4.2', date: '2026-04-18', title: 'СМР-мастер и исправления',
        commits: 40,
        changes: [
            { type: 'feat', text: '3-шаговый мастер заполнения СМР: Часы → Работы → Просмотр' },
            { type: 'feat', text: 'Учёт отработанных часов по бригадам и сотрудникам (новая таблица application_hours)' },
            { type: 'feat', text: 'Ролевая схема СМР: бригадир заполняет свою бригаду → прораб проверяет и одобряет' },
            { type: 'feat', text: 'Группировка заявок СМР на странице КП: по объектам, внутри — по датам' },
            { type: 'feat', text: 'Новый Excel-отчёт СМР: листы «Часы» и «Работы», столбец «Заполнил», без ЗП и цен' },
            { type: 'feat', text: 'Иконки Tabler Icons для техники и бригад (экскаватор, бульдозер, кран, погрузчик, самосвал и др.)' },
            { type: 'feat', text: 'Экран «Обновление системы» при недоступности API — автоперезагрузка при восстановлении' },
            { type: 'feat', text: 'Выбор доп. работ как раскрываемый список по категориям с поиском и единицами измерения' },
            { type: 'fix', text: 'Push-уведомления — поле aud в VAPID JWT теперь соответствует origin push-сервиса (Chrome/Firefox/Safari)' },
            { type: 'fix', text: 'Парсер каталога КП — единицы измерения читаются из колонки G, а не F (была число вместо «шт»/«м²»)' },
            { type: 'fix', text: 'Название и адрес объекта восстановлены на карточках канбана (enrich в /api/dashboard)' },
            { type: 'fix', text: 'Полная очистка при истечении сессии: cookies, localStorage, IndexedDB, Cache API — без зацикливания 401' },
            { type: 'fix', text: 'Время техники по умолчанию подставляется из настроек администратора (было жёстко 08:00–17:00)' },
            { type: 'fix', text: 'Иконка бригады сохраняется в БД и возвращается в /api/dashboard и /api/teams/{id}/details' },
            { type: 'fix', text: 'Отправка отчёта СМР прорабом больше не падает с KeyError: salary — цены ищутся на сервере' },
            { type: 'fix', text: 'Миграция иконок lucide → tabler в БД (users → usersGroup, cog → settings, и т. д.)' },
            { type: 'refactor', text: 'Страница КП: плоский список заменён на объединённые по объекту карточки' },
            { type: 'refactor', text: 'Каталог КП: очистка мусорных «nan» в единицах измерения + автоперерасбор xlsx на старте' },
        ],
    },
    {
        version: '2.4.1', date: '2026-04-17', title: 'Исправления и улучшения',
        commits: 22,
        changes: [
            { type: 'fix', text: 'Импорт СМР из PDF — ошибка path traversal на абсолютных путях' },
            { type: 'fix', text: 'Push-уведомления на iOS — VAPID JWT токен теперь валиден' },
            { type: 'fix', text: 'Настройки и Админка доступны из мобильного меню' },
            { type: 'fix', text: 'Единицы измерения в статистике объекта и истории заявок' },
            { type: 'fix', text: 'PDF-парсер — очистка адреса от заголовков таблицы и кавычек' },
            { type: 'fix', text: 'Иконка бригады обновляется на карточке после сохранения' },
            { type: 'fix', text: 'Вёрстка страницы объектов после создания на мобильном' },
            { type: 'fix', text: 'Уведомления группируются в журнале с раскрываемым списком получателей' },
            { type: 'fix', text: 'Кнопки карточек — только иконка на узких экранах, текст на lg+' },
            { type: 'fix', text: 'Все формы отправляются по Enter (хук useEnterToSubmit)' },
            { type: 'fix', text: 'Удаление аккаунта — скромная ссылка вместо громоздкой кнопки' },
            { type: 'fix', text: 'Поле «Отчество» — сокращённая подпись, подсказка в placeholder' },
            { type: 'fix', text: 'Сортировка пользователей в Админке — по рангу роли, затем по ФИО' },
            { type: 'fix', text: 'Admin-link 500: определение primary без отсутствующей колонки created_at' },
            { type: 'feat', text: 'Специальность рабочих синхронизируется с должностью в бригаде' },
            { type: 'feat', text: 'Все команды бота реализованы: /help, /profile, /myorders, /review, /debtors, /broadcast, /backup, /logs' },
            { type: 'feat', text: 'Статусы сотрудников в расстановке для всех (с учётом status_from и status_until)' },
            { type: 'feat', text: 'Кастомные иконки категорий техники и бригад в канбане и селекторах' },
            { type: 'feat', text: 'Кнопка «Установить приложение» в боковом меню — заметная карточка над «Гайд»' },
            { type: 'refactor', text: '«PWA» заменено на «Приложение» во всём интерфейсе' },
            { type: 'refactor', text: 'Нижняя панель: удалены Настройки/Админка — только через бургер-меню, увеличенный размер' },
            { type: 'refactor', text: 'Уведомления в журнале — одна запись на событие с полным списком получателей' },
        ],
    },
    {
        version: '2.4', date: '2026-04-17', title: 'Новые возможности',
        commits: 40,
        changes: [
            { type: 'feat', text: 'Установка приложения — баннер с определением браузера (Safari iOS, Chrome, Яндекс)' },
            { type: 'feat', text: 'Автоматическое обновление приложения — модалка с 30-сек обратным отсчётом' },
            { type: 'feat', text: 'Push-уведомления с типизированными иконками (10 типов: заявка одобрена, отклонена, СМР и т.д.)' },
            { type: 'feat', text: 'Страница «Настройки» для всех ролей — каналы, категории модератора, интерфейс' },
            { type: 'feat', text: 'Страница «Админка» для boss+ (замена «Система») — пользователи, журнал, рассылки, настройки' },
            { type: 'feat', text: 'Управление ролями пользователей в таблице Админки: inline-выбор с подтверждением' },
            { type: 'feat', text: 'Разделение ФИО на Фамилия / Имя / Отчество (с миграцией существующих записей)' },
            { type: 'feat', text: 'Настраиваемые уведомления модератора — новые заявки, должники СМР, запросы объектов, обмены' },
            { type: 'feat', text: 'Иконки для категорий техники и бригад — 24 иконки техники и 15 иконок бригад' },
            { type: 'feat', text: 'Виджет должников СМР можно скрыть (сессионно на виджете или глобально через Настройки)' },
            { type: 'feat', text: 'Единицы измерения из каталога КП — в статистике объекта и форме заполнения' },
            { type: 'feat', text: 'Бот: список команд при /start и неизвестной команде, с учётом роли' },
            { type: 'feat', text: 'Счётчики вкладок СМР как badge (вместо текста в заголовке — нет переносов на мобильном)' },
            { type: 'fix', text: 'Сохранение специальности пользователя в профиле' },
            { type: 'fix', text: 'Объект и адрес отображаются раздельно во всём UI (новый ObjectDisplay)' },
            { type: 'refactor', text: 'Прораб — только просмотр на странице объектов (скачивание файлов, статистика)' },
            { type: 'refactor', text: 'Бригадир/рабочий/водитель — без доступа к странице объектов' },
            { type: 'refactor', text: 'Нижняя панель — увеличенный размер, симметричные отступы, улучшенные иконки' },
        ],
    },
    {
        version: '2.3', date: '2026-04-16', title: 'Security Sprint — Production Hardening',
        commits: 18,
        changes: [
    ...
    },
    {
        ...
        changes: [
            ...
            { type: 'feat', text: 'Сохранение сессии в PWA — вход сохраняется при перезапуске приложения' },
            { type: 'feat', text: 'Редизайн страницы входа — тёмная тема, логотип, анимации' },
            { type: 'feat', text: 'ИИ-поддержка — расширенный список моделей, автоматический выбор доступной' },
            { type: 'feat', text: 'ИИ-ассистент отвечает с реальными данными: бригады, техника, заявки, коды приглашений' },
            { type: 'feat', text: 'Уведомления — колокольчик с бейджем непрочитанных в Sidebar' },
            { type: 'feat', text: 'Онлайн-пользователи — счётчик и список активных пользователей' },
            { type: 'feat', text: 'Интерактивный онбординг-гайд при первом входе с навигацией по страницам' },
            { type: 'feat', text: 'Создание заявки через бота /order — Telegram и MAX' },
            { type: 'feat', text: 'Статистика бригад — заявки, рабочие дни, объекты, топ прорабов' },
            { type: 'feat', text: 'Статистика техники — назначения, утилизация, объекты' },
            { type: 'feat', text: 'Файлы объектов — загрузка PDF, фото, Excel, Word с просмотром' },
            { type: 'feat', text: 'Импорт СМР из PDF — умное сравнение с каталогом, без дублей' },
            { type: 'feat', text: 'Группировка уведомлений в журнале действий' },
            { type: 'feat', text: 'Кнопка выхода из аккаунта в профиле' },
            { type: 'feat', text: 'Markdown-рендеринг в чате поддержки — таблицы, жирный, код' },
            { type: 'feat', text: 'Кастомный тонкий скроллбар по всему приложению' },
            { type: 'feat', text: 'Техника на канбан-карточках — тип + номер + время' },
            { type: 'feat', text: 'Имя водителя под техникой во всех карточках заявок' },
            { type: 'feat', text: 'Автоматическое ФИО водителя при регистрации по ссылке техники' },
            { type: 'feat', text: 'Настройки уведомлений по ролям — видны только релевантные категории' },
            { type: 'feat', text: 'Профиль в Sidebar — ФИО и роль вместо «Профиль»' },
            { type: 'feat', text: 'Статусы сотрудников — отпуск, больничный с датами' },
            { type: 'feat', text: 'Статусы в расстановке — Акт/Отп/Бол/Рем вместо 0/1' },
            { type: 'feat', text: 'PWA Push-уведомления — нативные уведомления на устройство' },
            { type: 'feat', text: 'Загрузочный экран при запуске приложения' },
            { type: 'feat', text: 'Предупреждение при назначении сотрудника без бригадира (СМР)' },
            { type: 'feat', text: 'Просмотр файлов — PDF/фото viewer с зумом и защитой от свайпов' },
            { type: 'fix', text: 'Мобильная верстка — горизонтальная прокрутка устранена' },
            { type: 'fix', text: 'Вертикальная прокрутка колесиком восстановлена' },
            { type: 'fix', text: 'PWA-сессия на iOS — тройное хранилище (localStorage + IndexedDB + Cache API)' },
            { type: 'fix', text: 'Авторизованные пользователи перенаправляются с Login на Dashboard' },
            { type: 'fix', text: 'Уведомления об обменах техники — переключатель работает корректно' },
            { type: 'fix', text: 'PDF-парсер — поддержка многостраничных документов' },
            { type: 'fix', text: 'Логи показывают названия объектов и заявок вместо ID' },
            { type: 'fix', text: 'Архивация объектов от имени пользователя, не системы' },
            { type: 'fix', text: 'Специальность пользователя сохраняется при редактировании' },
            { type: 'fix', text: 'Пользователи отсортированы по алфавиту внутри ролей' },
            { type: 'fix', text: 'Sidebar — логотип на всю ширину, активная вкладка корректно' },
            { type: 'fix', text: 'iPhone Dynamic Island — safe area padding' },
            { type: 'refactor', text: 'Code splitting — бандл разбит на 6 чанков (503KB → 270KB + vendor)' },
            { type: 'refactor', text: 'Карточки техники — кнопки спрятаны в «Параметры»' },
            { type: 'refactor', text: 'Карточки бригад — бригадир + кол-во участников + статистика' },
            { type: 'refactor', text: 'Ресурсы объекта — техника сгруппирована по категориям' },
            { type: 'refactor', text: 'План СМР — убраны столбцы ЗП и стоимость' },
            { type: 'refactor', text: 'Страница поддержки — компактные кнопки мессенджеров' },
            { type: 'refactor', text: 'Участники бригады сгруппированы по командам в заявках' },
        ],
    },
    {
        version: '2.5.1', date: '2026-04-14', title: 'Исправления навигации и логирование уведомлений',
        commits: 1,
        changes: [
            { type: 'fix', text: 'Кнопка Пользователи в боковом меню корректно прокручивает к секции' },
            { type: 'feat', text: 'Все уведомления записываются в журнал действий с ФИО получателя' },
            { type: 'fix', text: 'Условная загрузка Telegram SDK — ускорение загрузки в браузере в 30 раз' },
        ],
    },
    {
        version: '2.5.0', date: '2026-04-13', title: 'Подменю в боковой панели',
        commits: 3,
        changes: [
            { type: 'feat', text: 'Подменю в боковой панели с быстрым доступом к разделам' },
            { type: 'feat', text: 'Счётчики заявок и СМР в боковой панели (обновляются каждую минуту)' },
            { type: 'feat', text: 'Кнопка профиля перемещена вниз, кнопка свернуть вынесена за панель' },
            { type: 'feat', text: 'Переход к разделам страниц через URL-параметры из подменю' },
        ],
    },
    {
        version: '2.4.0', date: '2026-04-13', title: 'Бригадир + обязательная КП при создании объектов',
        commits: 2,
        changes: [
            { type: 'fix', text: 'Роль Бригадир отображается в журнале и списке пользователей' },
            { type: 'feat', text: 'Переработка подтверждения объектов — полная форма создания с обязательной КП' },
            { type: 'feat', text: 'КП обязательна при создании любого объекта' },
        ],
    },
    {
        version: '2.3.1', date: '2026-04-13', title: 'Полная ширина и лог поддержки',
        commits: 1,
        changes: [
    ...
    },
    {
        ...
        changes: [
            ...
            { type: 'feat', text: 'ИИ-поддержка обучена на полной базе знаний платформы (15 разделов)' },
            { type: 'feat', text: 'История чата поддержки сохраняется для каждого пользователя' },
            { type: 'feat', text: 'Уведомления суперадминам при обращении в поддержку' },
            { type: 'feat', text: 'Настройки ссылок поддержки и API ключа (только Супер-Админ)' },
            { type: 'fix', text: 'Переход на OpenRouter API для ИИ-ассистента' },
        ],
    },
    {
        version: '2.1.3', date: '2026-04-13', title: 'Статичные приглашения',
        commits: 1,
        changes: [
