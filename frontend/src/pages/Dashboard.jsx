...
export default function Dashboard() {
  ...
  // Добавили is_foreman в стейт нового участника
  ...
  const [appForm, setAppForm] = useState({ date_target: smartDates[0].val, object_address: '', time_start: '08', time_end: '17', team_id: '', members: [], equip_id: '0', comment: '' });
  ...
  useEffect(() => {
      if (appForm.team_id) {
          axios.get(`/api/teams/${appForm.team_id}/details`).then(res => {
              setTeamMembers(res.data.members); setAppForm(prev => ({ ...prev, members: res.data.members.map(m => m.id) }));
          });
      } else setTeamMembers([]);
  }, [appForm.team_id]);
  ...
  // УПРАВЛЕНИЕ СОСТАВОМ И БРИГАДИРАМИ
  const handleAddMember = async (e) => {
      e.preventDefault();
      try {
          const fd = new FormData(); fd.append('fio', newMember.fio); fd.append('position', newMember.position); fd.append('is_foreman', newMember.is_foreman ? 1 : 0); fd.append('tg_id', tgId);
          await axios.post(`/api/teams/${manageTeamData.id}/members/add`, fd);
          setNewMember({ fio: '', position: 'Рабочий', is_foreman: false });
          const res = await axios.get(`/api/teams/${manageTeamData.id}/details`); setManageTeamData(res.data); fetchData();
      } catch (err) { alert("Ошибка"); }
  };

  const handleToggleForeman = async (memberId, currentStatus) => {
      try {
          const fd = new FormData(); fd.append('is_foreman', currentStatus ? 0 : 1); fd.append('tg_id', tgId);
          await axios.post(`/api/teams/members/${memberId}/toggle_foreman`, fd);
          const res = await axios.get(`/api/teams/${manageTeamData.id}/details`); setManageTeamData(res.data); fetchData();
      } catch (err) { alert("Ошибка обновления статуса бригадира"); }
  };
  ...
  const handleCreateApp = async (e) => {
      ...
      try {
          const fd = new FormData(); fd.append('tg_id', tgId); fd.append('date_target', appForm.date_target); fd.append('object_address', appForm.object_address); fd.append('time_start', appForm.time_start); fd.append('time_end', appForm.time_end); fd.append('team_id', appForm.team_id); fd.append('equip_id', appForm.equip_id); fd.append('comment', appForm.comment); fd.append('selected_members', appForm.members.join(','));
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-800 dark:text-gray-100 pb-10 transition-colors duration-200">
      ...
      {/* УПРАВЛЕНИЕ БРИГАДОЙ СО ЗВЕЗДОЙ БРИГАДИРА */}
                            ...
                            <div key={m.id} className={`flex justify-between items-center p-3 rounded-lg border shadow-sm text-sm transition-colors ${m.is_foreman ? 'bg-yellow-50 dark:bg-yellow-900/20 border-yellow-200 dark:border-yellow-700/50' : 'bg-white dark:bg-gray-700 dark:border-gray-600'}`}>
                                <div>
                                    <p className="font-bold text-gray-800 dark:text-gray-200">
                                        {m.fio}
                                        {m.is_foreman && <span className="ml-2 bg-yellow-100 dark:bg-yellow-800 text-yellow-800 dark:text-yellow-200 text-[10px] uppercase font-extrabold px-2 py-0.5 rounded shadow-sm">⭐️ Бригадир</span>}
                                    </p>
                                ...
                                {showTeamManagement && (
                                    <div className="flex flex-col space-y-1">
                                        <button onClick={() => handleToggleForeman(m.id, m.is_foreman)} className={`font-bold px-2 py-1 rounded-md text-xs transition ${m.is_foreman ? 'bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-300' : 'bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-400 hover:bg-yellow-200'}`}>
                                            {m.is_foreman ? 'Снять статус' : '⭐️ Назначить'}
                                        </button>
                ...
                {showTeamManagement && (
                    <form onSubmit={handleAddMember} className="bg-blue-50 dark:bg-blue-900/20 p-5 rounded-xl border border-blue-100 dark:border-blue-800">
                        ...
                        <div className="flex items-center mb-4">
                            ...
                            <label htmlFor="is_foreman_cb" className="ml-2 text-sm font-bold text-gray-700 dark:text-gray-300 cursor-pointer">⭐️ Назначить этого человека бригадиром</label>
                        </div>
                        <button type="submit" className="w-full bg-blue-600 text-white py-2.5 rounded-lg text-sm font-medium hover:bg-blue-700 shadow-md">Добавить в состав</button>
                    </form>
                )}
            </div>
        </div>
      ...
      {/* ОСТАЛЬНЫЕ МОДАЛКИ (Без изменений, чтобы сократить текст, но они уже встроены выше в полном коде) */}
      {isAppModalOpen && (
            ...
            <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg my-8 shadow-2xl transition-colors">
                ...
                <form onSubmit={handleCreateApp} className="p-6 space-y-6 text-sm">
                            ...
                            <div className="mt-3 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-800/50">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {teamMembers.map(m => {
                                        ...
                                        return (<button key={m.id} type="button" onClick={() => toggleAppMember(m.id)} className={`px-3 py-1.5 text-sm font-medium rounded-lg border transition flex items-center ${isSelected ? 'bg-blue-600 text-white border-blue-700 shadow-md' : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:bg-gray-100'}`}>{isSelected ? <span className="mr-1">✓</span> : <span className="mr-1 opacity-0">✓</span>}{m.fio}</button>)
                                    })}
                                </div>
                            </div>
                        )}
                    ...
                    <div className="space-y-3">
                        ...
                        <div className="flex flex-wrap gap-2">
                            <button type="button" onClick={() => setAppForm({...appForm, equip_id: '0'})} className={`px-4 py-2 text-sm font-medium rounded-xl border transition ${appForm.equip_id === '0' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 shadow-sm' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 dark:border-gray-600 hover:bg-gray-50'}`}>❌ Без техники</button>
                            {data.equipment.map(e => (<button key={e.id} type="button" onClick={() => setAppForm({...appForm, equip_id: e.id})} className={`px-4 py-2 text-sm font-medium rounded-xl border transition ${Number(appForm.equip_id) === e.id ? 'bg-indigo-50 border-indigo-500 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 shadow-sm' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 dark:border-gray-600 hover:bg-gray-50'}`}>{e.name}</button>))}
                        </div>
                        {appForm.equip_id !== '0' && (
