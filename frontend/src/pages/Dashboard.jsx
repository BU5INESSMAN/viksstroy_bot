...
export default function Dashboard() {
  ...
  const handleDeleteUser = () => { setConfirmDialog({ isOpen: true, title: 'Удаление пользователя', text: `ПОЛНОСТЬЮ удалить «${profileData.fio}»?`, confirmText: 'Удалить', color: 'red', onConfirm: async () => { setConfirmDialog({ ...confirmDialog, isOpen: false }); try { const fd = new FormData(); fd.append('tg_id', tgId); await axios.post(`/api/users/${profileData.user_id}/delete`, fd); setProfileModalOpen(false); fetchData(); } catch (e) { alert("Ошибка"); } } }); };
  const openEquipModal = async () => { try { const res = await axios.get('/api/equipment/admin_list'); setEquipList(res.data); setEquipModalOpen(true); } catch (e) { alert("Ошибка загрузки"); } };
  ...
  if (displayCategories.length === 1) { displayCategories.unshift('Кран', 'Экскаватор'); }
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-800 dark:text-gray-100 pb-10 transition-colors duration-200">
      ...
      {/* МОДАЛКИ (Остаются такими же, как в прошлом ответе) */}
                        ...
                        {equipList.map(eq => (
                            <div key={eq.id} className={`flex flex-col sm:flex-row justify-between sm:items-center p-3 mb-2 rounded-lg border dark:border-gray-600 shadow-sm transition-colors ${eq.is_active ? 'bg-white dark:bg-gray-700' : 'bg-gray-100 dark:bg-gray-800 opacity-60'}`}>
                                ...
                                <div className="flex items-center space-x-2">
                                    <button onClick={() => handleToggleEquip(eq.id, eq.is_active)} className={`px-3 py-1.5 rounded text-xs font-bold ${eq.is_active ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 dark:text-yellow-500 hover:bg-yellow-200' : 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-500 hover:bg-green-200'}`}>{eq.is_active ? 'В ремонт' : 'В строй'}</button>
                    ...
                    <div className="mt-8">
                        ...
                        {profileLogs.length > 0 ? (
                            <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
                                {profileLogs.map(log => (<div key={log.id} className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg border border-gray-100 dark:border-gray-600 flex justify-between items-start text-sm"><span className="text-gray-700 dark:text-gray-300">{log.action}</span><span className="text-xs text-gray-400 dark:text-gray-500 whitespace-nowrap ml-4">{new Date(log.timestamp).toLocaleString('ru-RU', {day: '2-digit', month: 'short', hour: '2-digit', minute:'2-digit'})}</span></div>))}
                            </div>
        ...
        <div className="fixed inset-0 bg-gray-900 bg-opacity-60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-2xl w-full max-w-md transition-colors">
                ...
                <div className="space-y-4 mb-6">
                    <div><label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">✈️ Ссылка для Telegram:</label><button onClick={() => copyToClipboard(inviteInfo.tg_bot_link, 'tg')} className={`w-full text-left px-4 py-3 border rounded-lg text-sm transition ${copiedLink === 'tg' ? 'bg-green-50 dark:bg-green-900/30 border-green-500 text-green-700 dark:text-green-400' : 'bg-gray-50 dark:bg-gray-700 dark:border-gray-600 text-gray-600 dark:text-gray-300'}`}>{copiedLink === 'tg' ? '✅ Ссылка скопирована!' : '🔗 Нажмите, чтобы скопировать'}</button></div>
                    <div><label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">🌐 Web-ссылка:</label><button onClick={() => copyToClipboard(inviteInfo.invite_link, 'web')} className={`w-full text-left px-4 py-3 border rounded-lg text-sm transition ${copiedLink === 'web' ? 'bg-green-50 dark:bg-green-900/30 border-green-500 text-green-700 dark:text-green-400' : 'bg-gray-50 dark:bg-gray-700 dark:border-gray-600 text-gray-600 dark:text-gray-300'}`}>{copiedLink === 'web' ? '✅ Ссылка скопирована!' : '🔗 Нажмите, чтобы скопировать'}</button></div>
                </div>
      ...
      {isManageModalOpen && manageTeamData && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-40 overflow-y-auto">
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-xl w-full max-w-lg my-8 relative transition-colors">
                <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold">Бригада «{manageTeamData.name}»</h3><button onClick={() => setManageModalOpen(false)} className="text-gray-400 hover:text-gray-700 text-2xl">&times;</button></div>
                <div className="mb-6">
            ...
            <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl w-full max-w-md my-8 shadow-2xl transition-colors">
                ...
                <form onSubmit={handleCreateApp} className="space-y-4 text-sm">
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {data.equipment.map(e => (<button key={e.id} type="button" onClick={() => setAppForm({...appForm, equip_id: e.id})} className={`px-3 py-2 text-sm font-medium rounded-lg border transition ${Number(appForm.equip_id) === e.id ? 'bg-indigo-50 border-indigo-500 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400' : 'bg-white dark:bg-gray-700 dark:border-gray-600 text-gray-600 dark:text-gray-300'}`}>🚜 {e.name}</button>))}
                        </div>
                    </div>
                    ...
                    <div className="flex space-x-3 pt-4"><button type="button" onClick={() => setAppModalOpen(false)} className="w-1/3 bg-gray-100 dark:bg-gray-700 py-3 rounded-xl font-medium">Отмена</button><button type="submit" className="w-2/3 bg-blue-600 text-white py-3 rounded-xl font-medium shadow-md">Отправить заявку</button></div>
                </form>
            </div>
        </div>
      )}
