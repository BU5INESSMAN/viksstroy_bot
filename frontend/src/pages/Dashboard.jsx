...
// Функция для генерации умных кнопок дат
const getSmartDates = () => {
    ...
    return [0, 1, 2].map(i => {
        ...
        return {
            ...
            label: `${d.getDate().toString().padStart(2,'0')}.${(d.getMonth()+1).toString().padStart(2,'0')}, ${days[d.getDay()]} (${labels[i]})`
        };
    });
};
...
export default function Dashboard() {
  ...
  const fetchData = () => {
    axios.get('/api/dashboard').then(res => { setData(res.data); if(res.data.equip_categories?.length > 0 && !newEquip.category) setNewEquip(prev => ({...prev, category: res.data.equip_categories[0]})); }).catch(() => {});
  ...
  // Загрузка состава бригады при ее выборе в заявке
  useEffect(() => {
      if (appForm.team_id) {
          axios.get(`/api/teams/${appForm.team_id}/details`).then(res => {
              setTeamMembers(res.data.members);
              setAppForm(prev => ({ ...prev, members: res.data.members.map(m => m.id) })); // По умолчанию выбираем всех
          });
      } else setTeamMembers([]);
  }, [appForm.team_id]);

  const toggleAppMember = (id) => {
      setAppForm(prev => ({ ...prev, members: prev.members.includes(id) ? prev.members.filter(m => m !== id) : [...prev.members, id] }));
  };
  ...
  const handleCreateApp = async (e) => {
      e.preventDefault();
      if (!appForm.team_id) return alert("Пожалуйста, выберите бригаду!");
      if (appForm.members.length === 0) return alert("Выберите хотя бы одного рабочего на выезд!");
      ...
      try {
          ...
          fd.append('time_start', appForm.time_start);
          fd.append('time_end', appForm.time_end);
          fd.append('team_id', appForm.team_id);
          fd.append('equip_id', appForm.equip_id);
          fd.append('comment', appForm.comment);
          fd.append('selected_members', appForm.members.join(',')); // Передаем ID рабочих

          await axios.post('/api/applications/create', fd);
          setAppModalOpen(false); fetchData(); alert("Заявка успешно отправлена на модерацию!");
    ...
    <div className="bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-800 dark:text-gray-100 pb-10 transition-colors duration-200">
      <nav className="bg-white dark:bg-gray-800 shadow-sm border-b border-transparent dark:border-gray-700 px-4 sm:px-6 py-4 flex justify-between items-center mb-6 transition-colors duration-200">
        ...
        <div className="flex items-center space-x-2 sm:space-x-4">
          <button onClick={toggleTheme} className="text-xl w-10 h-10 flex items-center justify-center rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition">
            {themeIcon}
          </button>
          <button onClick={() => navigate('/guide')} className="flex items-center space-x-2 hover:bg-gray-50 dark:hover:bg-gray-700 px-3 py-1.5 rounded-lg transition border border-transparent hover:border-gray-200 dark:hover:border-gray-600">
             ...
             <span className="text-sm font-medium hidden md:block">Инструкция</span>
          </button>
          ...
          <span className="text-sm text-gray-500 dark:text-gray-400 hidden lg:block">Должность: <b className="text-gray-800 dark:text-gray-200">{roleNames[role]}</b></span>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        {/* ... (Статистика, Бригады, Активный наряд и Панель действий остаются без изменений, они работают идеально) ... */}
        {showStats && (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-100 dark:border-gray-700 transition-colors duration-200">
            ...
            {data.teams.length > 0 ? (
                ...
                {data.teams.map(t => (
                    <li key={t.id} className="p-4 bg-gray-50 dark:bg-gray-700/50 rounded-lg border border-gray-200 dark:border-gray-600 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
            ...
            {showTeamManagement && (<button onClick={() => setTeamModalOpen(true)} className="mt-5 w-full bg-gray-50 dark:bg-gray-700 border border-gray-300 dark:border-gray-600 py-2.5 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-600 font-medium">+ Создать новую бригаду</button>)}
          </div>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border-l-4 border-blue-500 relative transition-colors duration-200">
                    ...
                    {activeApp ? (
                        <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800 text-sm space-y-2">
                           ...
                           <p><b>Техника:</b> {activeApp.equip_name || 'Не требуется'}</p><p><b>Бригада:</b> {activeApp.team_name}</p>
                        </div>
        ...
        {/* ... (Остальной контент дашборда: Юзеры и Логи) ... */}
        {showUsersAndLogs && (
      ...
      {/* --- ОБНОВЛЕННАЯ МОДАЛКА: СОЗДАНИЕ ЗАЯВКИ --- */}
      {isAppModalOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center p-4 z-50 overflow-y-auto backdrop-blur-sm">
            <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg my-8 shadow-2xl transition-colors">
                <div className="flex justify-between items-center px-6 py-4 border-b dark:border-gray-700">
                    ...
                    <button onClick={() => setAppModalOpen(false)} className="text-gray-400 hover:text-gray-700 dark:hover:text-gray-200 text-2xl">&times;</button>
                </div>
                ...
                <form onSubmit={handleCreateApp} className="p-6 space-y-6 text-sm">
                    ...
                    <div className="space-y-4">
                        <div>
                            ...
                            <div className="flex flex-wrap gap-2">
                                {smartDates.map(d => (
                                    <button key={d.val} type="button" onClick={() => setAppForm({...appForm, date_target: d.val})} className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition ${appForm.date_target === d.val ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 border-blue-300 dark:border-blue-700' : 'bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:bg-gray-100'}`}>
                                        {d.label}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div>
                            ...
                            <input type="text" required value={appForm.object_address} onChange={e => setAppForm({...appForm, object_address: e.target.value})} placeholder="г. Москва, ул. Ленина, 10" className="w-full border dark:border-gray-600 bg-white dark:bg-gray-700 p-2.5 rounded-lg outline-none font-medium dark:text-white" />
                        </div>
                    </div>

                    <hr className="dark:border-gray-700" />

                    {/* Блок 2: Бригада и Люди */}
                    <div className="space-y-3">
                        <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">👥 Бригада</label>
                        <div className="flex flex-wrap gap-2">
                            {data.teams.map(t => (
                                <button key={t.id} type="button" onClick={() => setAppForm({...appForm, team_id: t.id})} className={`px-4 py-2 text-sm font-medium rounded-xl border transition ${Number(appForm.team_id) === t.id ? 'bg-blue-50 border-blue-500 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 shadow-sm' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 dark:border-gray-600 hover:bg-gray-50'}`}>
                                    🏗 {t.name}
                                </button>
                            ))}
                        </div>
                        ...
                        {teamMembers.length > 0 && (
                            <div className="mt-3 p-4 bg-blue-50 dark:bg-blue-900/10 rounded-xl border border-blue-100 dark:border-blue-800/50">
                                <label className="block text-xs font-bold text-blue-800 dark:text-blue-300 mb-3 uppercase tracking-wide">Выберите состав на выезд</label>
                                <div className="flex flex-wrap gap-2">
                                    {teamMembers.map(m => {
                                        ...
                                        return (
                                            <button key={m.id} type="button" onClick={() => toggleAppMember(m.id)} className={`px-3 py-1.5 text-sm font-medium rounded-lg border transition flex items-center ${isSelected ? 'bg-blue-600 text-white border-blue-700 shadow-md' : 'bg-white dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:bg-gray-100'}`}>
                                                {isSelected ? <span className="mr-1">✓</span> : <span className="mr-1 opacity-0">✓</span>}
                                                {m.fio}
                                            </button>
                                        )
                                    })}
                    ...
                    {/* Блок 3: Техника и Время */}
                    <div className="space-y-3">
                        <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase">🚜 Требуемая техника</label>
                        <div className="flex flex-wrap gap-2">
                            <button type="button" onClick={() => setAppForm({...appForm, equip_id: '0'})} className={`px-4 py-2 text-sm font-medium rounded-xl border transition ${appForm.equip_id === '0' ? 'bg-indigo-50 border-indigo-500 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 shadow-sm' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 dark:border-gray-600 hover:bg-gray-50'}`}>
                                ❌ Без техники
                            </button>
                            {data.equipment.map(e => (
                                <button key={e.id} type="button" onClick={() => setAppForm({...appForm, equip_id: e.id})} className={`px-4 py-2 text-sm font-medium rounded-xl border transition ${Number(appForm.equip_id) === e.id ? 'bg-indigo-50 border-indigo-500 text-indigo-700 dark:bg-indigo-900/30 dark:text-indigo-400 shadow-sm' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 dark:border-gray-600 hover:bg-gray-50'}`}>
                                    {e.name}
                                </button>
                            ))}
                        </div>
                        ...
                        {appForm.equip_id !== '0' && (
                            <div className="mt-3 p-4 bg-indigo-50 dark:bg-indigo-900/10 rounded-xl border border-indigo-100 dark:border-indigo-800/50">
                                <label className="block text-xs font-bold text-indigo-800 dark:text-indigo-300 mb-3 uppercase tracking-wide">Время работы техники</label>
                                <div className="flex items-center space-x-3">
                                    <div className="flex items-center bg-white dark:bg-gray-800 border dark:border-gray-600 rounded-lg overflow-hidden shadow-sm">
                                        <span className="bg-gray-100 dark:bg-gray-700 px-3 py-2 text-xs font-bold text-gray-500 border-r dark:border-gray-600">С</span>
                                        <input type="number" min="0" max="23" required value={appForm.time_start} onChange={e => setAppForm({...appForm, time_start: e.target.value})} className="w-14 text-center py-2 font-bold focus:outline-none dark:bg-gray-800 dark:text-white" />
                                        <span className="pr-3 font-bold text-gray-400">:00</span>
                                    </div>
                                    <span className="text-gray-400 font-bold">—</span>
                                    <div className="flex items-center bg-white dark:bg-gray-800 border dark:border-gray-600 rounded-lg overflow-hidden shadow-sm">
                                        <span className="bg-gray-100 dark:bg-gray-700 px-3 py-2 text-xs font-bold text-gray-500 border-r dark:border-gray-600">ДО</span>
                                        <input type="number" min="0" max="23" required value={appForm.time_end} onChange={e => setAppForm({...appForm, time_end: e.target.value})} className="w-14 text-center py-2 font-bold focus:outline-none dark:bg-gray-800 dark:text-white" />
                                        <span className="pr-3 font-bold text-gray-400">:00</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <hr className="dark:border-gray-700" />
                    <div>
                        <label className="block text-xs font-bold text-gray-500 dark:text-gray-400 mb-2 uppercase">💬 Комментарий</label>
                        <input type="text" value={appForm.comment} onChange={e => setAppForm({...appForm, comment: e.target.value})} placeholder="Дополнительная информация..." className="w-full border dark:border-gray-600 bg-white dark:bg-gray-700 p-2.5 rounded-lg outline-none dark:text-white" />
                    </div>

                    <div className="flex space-x-3 pt-2">
                        <button type="button" onClick={() => setAppModalOpen(false)} className="w-1/3 bg-gray-100 dark:bg-gray-700 py-3.5 rounded-xl font-bold text-gray-700 dark:text-gray-300">Отмена</button>
                        <button type="submit" className="w-2/3 bg-blue-600 text-white py-3.5 rounded-xl font-bold shadow-lg hover:bg-blue-700 transition">Отправить заявку</button>
                    </div>
                </form>
            </div>
        </div>
      ...
      {inviteInfo && (<div className="fixed inset-0 bg-gray-900 bg-opacity-60 backdrop-blur-sm flex items-center justify-center p-4 z-50"><div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-2xl w-full max-w-md"><h3 className="text-xl font-bold mb-6 text-center">Приглашение</h3><div className="space-y-4 mb-6"><div><label className="block text-sm font-bold text-gray-700 mb-1">✈️ Telegram:</label><button onClick={() => copyToClipboard(inviteInfo.tg_bot_link, 'tg')} className="w-full text-left px-4 py-3 border rounded-lg text-sm bg-gray-50">{copiedLink === 'tg' ? '✅ Скопировано!' : '🔗 Копировать'}</button></div></div><button onClick={() => setInviteInfo(null)} className="w-full bg-gray-800 text-white py-3 rounded-xl font-medium">Закрыть</button></div></div>)}
      {isManageModalOpen && manageTeamData && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-40 overflow-y-auto"><div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-xl w-full max-w-lg my-8"><div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold">Бригада «{manageTeamData.name}»</h3><button onClick={() => setManageModalOpen(false)} className="text-gray-400 text-2xl">&times;</button></div><div className="mb-6"><div className="max-h-60 overflow-y-auto space-y-2 border rounded-xl p-3 bg-gray-50 dark:bg-gray-900/50">{manageTeamData.members.map(m => (<div key={m.id} className="flex justify-between items-center bg-white dark:bg-gray-700 p-3 rounded-lg border shadow-sm text-sm"><div><p className="font-bold">{m.fio}</p><p className="text-xs text-gray-500 uppercase">{m.position}</p></div>{showTeamManagement && (<button onClick={() => handleDeleteMember(m.id, m.fio)} className="text-red-500 font-bold px-3 py-1.5 bg-red-50 rounded-lg">Удалить</button>)}</div>))}</div></div></div></div>)}
      {isTeamModalOpen && (<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50"><div className="bg-white dark:bg-gray-800 p-6 rounded-2xl w-full max-w-sm"><h3 className="text-xl font-bold mb-4 text-center">Новая бригада</h3><form onSubmit={handleCreateTeam}><input type="text" required value={newTeamName} onChange={e => setNewTeamName(e.target.value)} placeholder="Название" className="w-full px-3 py-3 border dark:bg-gray-700 rounded-xl mb-4 outline-none" /><div className="flex space-x-2"><button type="button" onClick={() => setTeamModalOpen(false)} className="w-1/2 bg-gray-100 dark:bg-gray-700 py-3 rounded-xl font-medium">Отмена</button><button type="submit" className="w-1/2 bg-blue-600 text-white py-3 rounded-xl font-medium">Создать</button></div></form></div></div>)}
    </div>
  );
}
