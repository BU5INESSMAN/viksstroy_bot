...
export default function Dashboard() {
  ...
  const handleReviewApp = async (appId, status) => { if (!window.confirm(status === 'approved' ? 'Одобрить заявку?' : 'Отклонить заявку?')) return; try { const fd = new FormData(); fd.append('new_status', status); fd.append('tg_id', tgId); await axios.post(`/api/applications/${appId}/review`, fd); fetchData(); } catch (err) { alert("Ошибка при обновлении статуса"); } };
  ...
  const handleToggleForeman = async (memberId, currentStatus) => { try { const fd = new FormData(); fd.append('is_foreman', currentStatus ? 0 : 1); fd.append('tg_id', tgId); await axios.post(`/api/teams/members/${memberId}/toggle_foreman`, fd); const res = await axios.get(`/api/teams/${manageTeamData.id}/details`); setManageTeamData(res.data); fetchData(); } catch (err) { alert("Ошибка обновления статуса"); } };
  ...
  const handleCreateApp = async (e) => { e.preventDefault(); if (!appForm.team_id) return alert("Выберите бригаду!"); if (appForm.members.length === 0) return alert("Выберите хотя бы одного рабочего!"); try { const fd = new FormData(); fd.append('tg_id', tgId); fd.append('date_target', appForm.date_target); fd.append('object_address', appForm.object_address); fd.append('team_id', appForm.team_id); fd.append('comment', appForm.comment); fd.append('selected_members', appForm.members.join(',')); fd.append('equipment_data', JSON.stringify(appForm.equipment)); await axios.post('/api/applications/create', fd); setAppModalOpen(false); fetchData(); alert("Отправлено на модерацию!"); } catch (err) { alert("Ошибка"); } };
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-800 dark:text-gray-100 pb-24 transition-colors duration-200">
                        ...
                        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border-l-4 border-blue-500 relative transition-colors duration-200 h-fit">
                            ...
                            {activeApp ? (
                                <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-100 dark:border-blue-800 text-sm space-y-2">
                                   <p><b>Дата:</b> {activeApp?.date_target}</p><p><b>Объект:</b> {activeApp?.object_address}</p><p><b>Техника:</b> {activeEquipText}</p><p><b>Бригада:</b> {activeApp?.team_name}</p>
                                </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 z-40 flex justify-around items-center pb-safe shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)] transition-colors">
          ...
          {showUsersAndLogs && (
              <button onClick={() => setActiveTab('system')} className={`flex flex-col items-center py-3 w-full transition-colors ${activeTab === 'system' ? 'text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200'}`}><span className="text-2xl mb-1">⚙️</span><span className="text-[10px] font-bold uppercase tracking-wide">Система</span></button>
          )}
      </div>
...
function StatCard({ title, value, color, text = "text-gray-900 dark:text-gray-100" }) {
  ...
  return (
      <div className={`bg-white dark:bg-gray-800 p-5 rounded-2xl shadow-sm border-l-4 ${borders[color]} transition-colors`}><p className="text-sm font-medium text-gray-500 dark:text-gray-400 mb-1 uppercase">{title}</p><p className={`text-3xl font-bold ${text}`}>{value}</p></div>
  );
}