...
export default function Dashboard() {
  ...
  const handleReviewApp = async (appId, status) => {
      ...
      try {
          const fd = new FormData(); fd.append('new_status', status); fd.append('tg_id', tgId);
  ...
  const handleToggleEquip = async (id, currentStatus) => {
      try {
          const fd = new FormData();
          fd.append('is_active', currentStatus ? 0 : 1);
          fd.append('tg_id', tgId);
  ...
  const handleDeleteEquip = (id, name) => {
      ...
      try {
          ...
          axios.post(`/api/equipment/${id}/delete`, fd).then(() => {
              axios.get('/api/equipment/admin_list').then(res => setEquipList(res.data || []));
              fetchData();
          });
      } catch (e) { alert("Ошибка"); }
  };
  ...
  const handlePublishAppsClick = () => {
      setConfirmDialog({
          isOpen: true,
          title: 'Публикация',
          text: 'Отправить все одобренные наряды в чат?',
          ...
          onConfirm: async () => {
              ...
              try {
                  ...
                  alert(`Опубликовано: ${res.data.published}`);
                  fetchData();
              } catch (err) {
  ...
  const approvedAppsCount = reviewApps.filter(a => a.status === 'approved').length;
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-800 dark:text-gray-100 pb-24 transition-colors duration-200">
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm p-6 border border-gray-100 dark:border-gray-700 transition-colors duration-200">
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
                    ...
                    {reviewApps.filter(a => a.status === 'waiting').length > 0 && (
                        <span className="mt-2 sm:mt-0 bg-yellow-100 dark:bg-yellow-900/40 text-yellow-800 dark:text-yellow-400 text-xs px-3 py-1.5 rounded-full font-bold border border-yellow-200 dark:border-yellow-700/50">
                            Ожидают: {reviewApps.filter(a => a.status === 'waiting').length}
                        </span>
                    )}
                </div>
                        ...
                        reviewApps.map(app => (
                            <div key={app.id} className="p-4 bg-gray-50 dark:bg-gray-700/30 rounded-xl border border-gray-200 dark:border-gray-600 flex flex-col md:flex-row justify-between gap-4 transition-colors">
                                <div className="text-sm space-y-1.5">
                                    ...
                                    <p><span className="text-gray-500 dark:text-gray-400">Бригада:</span> <b className="dark:text-white">{app.team_name}</b> (Прораб: {app.foreman_name})</p>
                                ...
                                <div className="flex flex-col items-end justify-between min-w-[140px] border-t dark:border-gray-600 md:border-t-0 pt-3 md:pt-0">
                                    {app.status === 'waiting' ? (
                                        <span className="bg-yellow-100 dark:bg-yellow-900/40 text-yellow-700 dark:text-yellow-400 text-xs font-bold px-3 py-1.5 rounded-lg mb-3 shadow-sm border border-yellow-200 dark:border-yellow-700/50">Ожидает проверки</span>
                                    ) : (
                                        <span className="bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-400 text-xs font-bold px-3 py-1.5 rounded-lg mb-3 shadow-sm border border-green-200 dark:border-green-700/50">Одобрено</span>
                                    )}

                                    {role === 'moderator' && app.status === 'waiting' && (
                                        <div className="flex w-full space-x-2 mt-auto">
                                            <button onClick={() => handleReviewApp(app.id, 'rejected')} className="flex-1 bg-red-100 hover:bg-red-200 text-red-700 dark:bg-red-900/40 dark:hover:bg-red-900/60 dark:text-red-400 py-2 rounded-lg font-bold transition border border-red-200 dark:border-red-800">❌</button>
                ...
                <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-lg shadow-2xl relative transition-colors overflow-hidden">
                    ...
                    <form onSubmit={handleCreateApp} className="p-6 space-y-6 text-sm">
                        ...
                        <div className="space-y-4">
                            <div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {smartDates.map(d => (
                                        <button key={d.val} type="button" onClick={() => setAppForm({...appForm, date_target: d.val})} className={`px-3 py-1.5 text-xs font-bold rounded-lg border transition ${appForm.date_target === d.val ? 'bg-blue-100 dark:bg-blue-900/40 text-blue-700 dark:text-blue-300 border-blue-300 dark:border-blue-700 shadow-sm' : 'bg-gray-50 dark:bg-gray-800 text-gray-500 dark:text-gray-400 border-gray-200 dark:border-gray-700 hover:bg-gray-100 dark:hover:bg-gray-700'}`}>
                                            {d.label}
                                        </button>
                                    ))}
                                </div>
                            ...
                            <div>
                                ...
                                <input type="text" required value={appForm.object_address} onChange={e => setAppForm({...appForm, object_address: e.target.value})} placeholder="г. Москва, ул. Ленина, 10" className="w-full border dark:border-gray-600 bg-white dark:bg-gray-700 p-2.5 rounded-lg outline-none font-medium dark:text-white shadow-sm" />
                            </div>
                        </div>
                        ...
                        <div className="space-y-3">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {data?.teams?.map(t => (
                                    <button key={t.id} type="button" onClick={() => setAppForm({...appForm, team_id: t.id})} className={`px-4 py-2 text-sm font-medium rounded-xl border transition ${Number(appForm.team_id) === t.id ? 'bg-blue-50 border-blue-500 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400 shadow-sm' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 dark:border-gray-600 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
                                        🏗 {t.name}
                                    </button>
                                ))}
                            </div>
                        ...
                        <div>
                            ...
                            <input type="text" value={appForm.comment} onChange={e => setAppForm({...appForm, comment: e.target.value})} placeholder="Дополнительная информация..." className="w-full border dark:border-gray-600 bg-white dark:bg-gray-700 p-3 rounded-lg outline-none dark:text-white shadow-sm" />
                        </div>

                        <div className="flex space-x-3 pt-4">
                            <button type="button" onClick={() => setAppModalOpen(false)} className="w-1/3 bg-gray-100 dark:bg-gray-700 py-3.5 rounded-xl font-bold text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-600 transition">Отмена</button>
                            <button type="submit" className="w-2/3 bg-blue-600 text-white py-3.5 rounded-xl font-bold shadow-lg hover:bg-blue-700 transition transform hover:scale-[1.02]">Отправить заявку</button>
                        </div>
                    </form>
                </div>
            </div>
            ...
            <div className="flex min-h-screen items-start justify-center p-4 pt-10 pb-24">
                <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-2xl shadow-2xl overflow-hidden transition-colors">
                    <div className="flex justify-between items-center px-6 py-4 bg-gray-800 dark:bg-gray-900 text-white">
                        ...
                        <button onClick={() => setEquipModalOpen(false)} className="text-gray-300 hover:text-white text-3xl leading-none">&times;</button>
                    </div>
                    <div className="p-6">
                        <div className="mb-6 max-h-64 overflow-y-auto border dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-900/50 p-2">
                            ...
                            {equipList?.map(eq => (
                                <div key={eq.id} className={`flex flex-col sm:flex-row justify-between sm:items-center p-3 mb-2 rounded-lg border dark:border-gray-600 shadow-sm transition-colors ${eq.is_active ? 'bg-white dark:bg-gray-700' : 'bg-gray-100 dark:bg-gray-800 opacity-60'}`}>
                                    ...
                                    <div className="flex items-center space-x-2">
                                        <button onClick={() => handleToggleEquip(eq.id, eq.is_active)} className={`px-3 py-1.5 rounded text-xs font-bold ${eq.is_active ? 'bg-yellow-100 dark:bg-yellow-900/30 text-yellow-700 hover:bg-yellow-200' : 'bg-green-100 dark:bg-green-900/30 text-green-700 hover:bg-green-200'}`}>
                                            {eq.is_active ? 'В ремонт' : 'В строй'}
                                        </button>
                                        <button onClick={() => handleDeleteEquip(eq.id, eq.name)} className="px-3 py-1.5 bg-red-50 dark:bg-red-900/30 text-red-600 hover:bg-red-100 rounded text-xs font-bold">Удалить</button>
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="space-y-4">
                            ...
                            {canEditUsers && (
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        <button type="button" onClick={() => setEditProfile({...editProfile, team_id: ''})} className={`px-4 py-2 text-sm font-medium rounded-lg border transition ${!editProfile.team_id ? 'bg-red-50 border-red-500 text-red-700' : 'bg-white dark:bg-gray-700 text-gray-600'}`}>
                                            Без бригады
                                        </button>
                                        {data?.teams?.map(t => (
                                            <button key={t.id} type="button" onClick={() => setEditProfile({...editProfile, team_id: t.id})} className={`px-4 py-2 text-sm font-medium rounded-lg border transition ${Number(editProfile.team_id) === t.id ? 'bg-blue-50 border-blue-500 text-blue-700' : 'bg-white dark:bg-gray-700 text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600'}`}>
                                                🏗 {t.name}
                                            </button>
                                        ))}
                                    </div>
                                ...
                                <div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {Object.entries(roleNames).filter(([key]) => key !== 'Гость').map(([key, label]) => (
                                            <button key={key} type="button" onClick={() => setEditProfile({...editProfile, role: key})} className={`px-3 py-2 text-sm font-medium rounded-lg border transition ${editProfile.role === key ? 'bg-indigo-50 border-indigo-500 text-indigo-700' : 'bg-white dark:bg-gray-700 text-gray-600 hover:bg-gray-50 dark:hover:bg-gray-600'}`}>
                                                {label}
                                            </button>
                                        ))}
                                    </div>
                            ...
                            {canEditUsers && (
                                <div className="flex justify-between items-center pt-4 mt-2 border-t dark:border-gray-700">
                                    <button onClick={handleDeleteUser} disabled={profileData.user_id === Number(tgId)} className="text-red-500 font-bold text-sm bg-red-50 px-4 py-2.5 rounded-lg disabled:opacity-50 hover:bg-red-100 transition">🗑 Удалить</button>
                                ...
                                <div className="space-y-3 max-h-64 overflow-y-auto pr-2">
                                    {profileLogs.map(log => (
                                        <div key={log.id} className="bg-gray-50 dark:bg-gray-700 p-3 rounded-lg border border-gray-100 dark:border-gray-600 flex justify-between items-start text-sm">
                                            ...
                                            <span className="text-xs text-gray-400 whitespace-nowrap ml-4">{log.timestamp ? new Date(log.timestamp).toLocaleString('ru-RU') : ''}</span>
                                        </div>
                                    ))}
                                </div>
                            ) : (
      ...
      {confirmDialog.isOpen && (
          <div className="fixed inset-0 z-[110] bg-black/60 overflow-y-auto backdrop-blur-sm">
              <div className="flex min-h-screen items-center justify-center p-4">
                  <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl w-full max-w-sm text-center shadow-2xl transition-colors">
                      ...
                      <div className="flex space-x-3">
                          <button onClick={() => setConfirmDialog({ ...confirmDialog, isOpen: false })} className="w-1/2 bg-gray-100 dark:bg-gray-700 py-3 rounded-xl font-bold text-gray-700 dark:text-gray-300">Отмена</button>
                          <button onClick={confirmDialog.onConfirm} className={`w-1/2 text-white py-3 rounded-xl font-bold shadow-md ${confirmDialog.color === 'red' ? 'bg-red-500 hover:bg-red-600' : 'bg-emerald-500 hover:bg-emerald-600'}`}>{confirmDialog.confirmText}</button>
                      </div>
                  </div>
              </div>
          </div>
              ...
              <div className="flex min-h-screen items-center justify-center p-4">
                  <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-2xl w-full max-w-md">
                      ...
                      <div className="space-y-4 mb-6">
                          <div>
                              <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1">✈️ Telegram:</label>
                              <button onClick={() => copyToClipboard(inviteInfo.tg_bot_link, 'tg')} className="w-full text-left px-4 py-3 border rounded-lg text-sm bg-gray-50">{copiedLink === 'tg' ? '✅ Скопировано!' : '🔗 Копировать'}</button>
                          </div>
                      </div>
                      <button onClick={() => setInviteInfo(null)} className="w-full bg-gray-800 text-white py-3 rounded-xl font-bold">Закрыть</button>
                  </div>
              </div>
          </div>
      )}
              ...
              <div className="flex min-h-screen items-center justify-center p-4">
                  <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl w-full max-w-sm transition-colors">
                      ...
                      <form onSubmit={handleCreateTeam}>
                          <input type="text" required value={newTeamName} onChange={e => setNewTeamName(e.target.value)} placeholder="Название" className="w-full px-3 py-3 border dark:bg-gray-700 rounded-xl mb-4 outline-none" />
                          <div className="flex space-x-2">
                              <button type="button" onClick={() => setTeamModalOpen(false)} className="w-1/2 bg-gray-100 dark:bg-gray-700 py-3 rounded-xl font-bold">Отмена</button>
                              <button type="submit" className="w-1/2 bg-blue-600 text-white py-3 rounded-xl font-bold shadow-md">Создать</button>
                          </div>
                      </form>
                  </div>
              </div>
