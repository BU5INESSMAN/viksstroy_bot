...
export default function Dashboard() {
  ...
  // НОВОЕ СОСТОЯНИЕ: ЗАЯВКИ НА РАССМОТРЕНИИ
  ...
  const [appForm, setAppForm] = useState({
      date_target: smartDates[0].val, object_address: '', team_id: '', members: [], equipment: [], comment: ''
  });
  ...
  const toggleEquipmentSelection = (equip) => { setAppForm(prev => { const exists = prev.equipment.find(e => e.id === equip.id); if (exists) return { ...prev, equipment: prev.equipment.filter(e => e.id !== equip.id) }; return { ...prev, equipment: [...prev.equipment, { id: equip.id, name: equip.name, time_start: '08', time_end: '17' }] }; }); };
  ...
  const handleCreateApp = async (e) => {
      ...
      try {
          ...
          fd.append('team_id', appForm.team_id); fd.append('comment', appForm.comment); fd.append('selected_members', appForm.members.join(','));
  ...
  const handlePublishAppsClick = () => { setConfirmDialog({ isOpen: true, title: 'Публикация', text: 'Отправить все одобренные наряды в чат?', confirmText: 'Опубликовать', color: 'green', onConfirm: async () => { setConfirmDialog({ ...confirmDialog, isOpen: false }); try { const fd = new FormData(); fd.append('tg_id', tgId); const res = await axios.post('/api/applications/publish', fd); alert(`Опубликовано: ${res.data.published}`); fetchData(); } catch (err) { alert("Ошибка: " + (err.response?.data?.detail || "")); } }}); };
  ...
  const showReviewPanel = ['moderator', 'boss', 'superadmin'].includes(role); // НОВАЯ ПАНЕЛЬ
  ...
  // Подсчет количества одобренных заявок для красного бейджа на кнопке "Отправить"
  ...
  return (
    <div className="bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-800 dark:text-gray-100 pb-10 transition-colors duration-200">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="space-y-6">
            ...
            {/* ПАНЕЛЬ ДЕЙСТВИЙ (С КРАСНЫМ БЕЙДЖЕМ) */}
        ...
        {/* --- НОВЫЙ БЛОК: ЗАЯВКИ НА РАССМОТРЕНИИ --- */}
        {showReviewPanel && (
                        ...
                        reviewApps.map(app => (
                            <div key={app.id} className="p-4 bg-gray-50 dark:bg-gray-700/30 rounded-xl border border-gray-200 dark:border-gray-600 flex flex-col md:flex-row justify-between gap-4 transition-colors">
                                ...
                                <div className="flex flex-col items-end justify-between min-w-[140px] border-t dark:border-gray-600 md:border-t-0 pt-3 md:pt-0">
                                    ...
                                    {/* Кнопки только для модератора, если статус waiting */}
      ...
      {/* 1. СОЗДАНИЕ ЗАЯВКИ (ИСПРАВЛЕН CSS) */}
      {isAppModalOpen && (
        ...
        <div className="fixed inset-0 z-[100] bg-black/60 overflow-y-auto backdrop-blur-sm">
            <div className="flex min-h-screen items-start justify-center p-4 pt-10 pb-24">
                <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-xl w-full max-w-lg relative transition-colors">
                    <div className="flex justify-between items-center mb-6"><h3 className="text-xl font-bold">Бригада «{manageTeamData?.name}»</h3><button onClick={() => setManageModalOpen(false)} className="text-gray-400 hover:text-gray-700 text-3xl leading-none">&times;</button></div>
                    <div className="mb-6">
