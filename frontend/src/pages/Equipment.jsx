...
export default function Equipment() {
    ...
    const fetchData = async () => {
        try {
            const [equipRes, dashRes] = await Promise.all([
                axios.get('/api/equipment/admin_list'),
                axios.get('/api/dashboard')
            ]);
            setEquipment(equipRes.data || []);
            const cats = dashRes.data?.equip_categories || [];
            setCategories(cats);
            if (cats.length > 0 && !newEquip.category) {
                setNewEquip(prev => ({ ...prev, category: cats[0] }));
            }
            setLoading(false);
        } catch (e) {
            console.error(e);
    ...
    const handleAddSingle = async (e) => {
        ...
        try {
            ...
            setNewEquip({ name: '', driver: '', category: categories[0] || '' });
            setCustomCategory('');
    ...
    const handleUpdateEquip = async (e) => {
        ...
        try {
            ...
            fd.append('category', selectedEquip.category);
            fd.append('driver', selectedEquip.driver);
            fd.append('status', selectedEquip.status);
            fd.append('photo_url', selectedEquip.photo_url || '');
            fd.append('tg_id', tgId);

            await axios.post(`/api/equipment/${selectedEquip.id}/update`, fd);
            alert("Сохранено!");
    ...
    const handleDeleteEquip = async () => {
        ...
        try {
            const fd = new FormData();
            fd.append('tg_id', tgId);
            await axios.post(`/api/equipment/${selectedEquip.id}/delete`, fd);
            setSelectedEquip(null);
            fetchData();
    ...
    const handleUnlinkDriver = async () => {
        ...
        try {
            const fd = new FormData();
            fd.append('tg_id', tgId);
            await axios.post(`/api/equipment/${selectedEquip.id}/unlink`, fd);
            alert("Водитель отвязан!");
            setSelectedEquip({ ...selectedEquip, tg_id: null });
            fetchData();
        } catch (err) { alert("Ошибка при отвязке"); }
    };
    ...
    const getStatusColor = (status) => {
        if (status === 'work') return 'bg-blue-100 text-blue-700 border-blue-200';
        if (status === 'repair') return 'bg-red-100 text-red-700 border-red-200';
        ...
        <div className="bg-gray-100 dark:bg-gray-900 min-h-screen text-gray-800 dark:text-gray-100 pb-10 transition-colors duration-200 font-sans">
            ...
            <div className="bg-white dark:bg-gray-800 shadow-sm border-b dark:border-gray-700 px-6 py-4 flex justify-between items-center sticky top-0 z-10">
                <h1 className="text-xl font-bold text-blue-600 dark:text-blue-400 flex items-center">
                    <span className="mr-2 text-2xl">🚜</span> Автопарк
                </h1>
                <button onClick={() => navigate('/dashboard')} className="px-4 py-2 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg font-bold transition text-sm">
                    ⬅ Назад
                </button>
            </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
                        {equipment.map(eq => (
                            <div key={eq.id} onClick={() => setSelectedEquip(eq)} className="bg-white dark:bg-gray-800 rounded-2xl p-4 border border-gray-200 dark:border-gray-700 shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all cursor-pointer group flex items-center space-x-4">
                                <div className="w-16 h-16 rounded-xl bg-gray-100 dark:bg-gray-700 bg-cover bg-center flex-shrink-0 flex items-center justify-center border border-gray-200 dark:border-gray-600" style={{ backgroundImage: eq.photo_url ? `url(${eq.photo_url})` : 'none' }}>
                                    {!eq.photo_url && <span className="text-3xl opacity-50">🚜</span>}
                                </div>
                                <div className="flex-1 overflow-hidden">
                                    <p className="font-bold text-gray-900 dark:text-white truncate">{eq.name}</p>
                                    <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 mb-2 truncate">{eq.driver || 'Водитель не указан'}</p>
                                    <span className={`text-[10px] font-extrabold uppercase px-2 py-0.5 rounded-md border ${getStatusColor(eq.status)}`}>
                    ...
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 max-w-2xl">
                        ...
                        <form onSubmit={handleAddSingle} className="space-y-5">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    <label className="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-1">Название техники <span className="text-red-500">*</span></label>
                                    <input type="text" required value={newEquip.name} onChange={e => setNewEquip({...newEquip, name: e.target.value})} placeholder="Напр.: Кран Ивановец 25т" className="w-full p-3 border dark:border-gray-600 bg-gray-50 dark:bg-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" />
                                </div>
                                <div>
                                    <label className="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-1">ФИО водителя</label>
                                    <input type="text" value={newEquip.driver} onChange={e => setNewEquip({...newEquip, driver: e.target.value})} placeholder="Напр.: Иванов И.И." className="w-full p-3 border dark:border-gray-600 bg-gray-50 dark:bg-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" />
                                </div>
                            </div>
                            <div>
                                <label className="block text-sm font-bold text-gray-600 dark:text-gray-300 mb-2">Категория</label>
                                <div className="flex flex-wrap gap-2">
                                    {displayCategories.map(cat => (
                                        <button key={cat} type="button" onClick={() => setNewEquip({...newEquip, category: cat})} className={`px-4 py-2 text-sm font-bold rounded-xl border transition-colors ${newEquip.category === cat ? 'bg-blue-100 text-blue-700 border-blue-300 dark:bg-blue-900/40 dark:text-blue-300' : 'bg-white dark:bg-gray-800 text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-gray-700'}`}>
                                            {cat}
                                        </button>
                                    ))}
                                </div>
                                {newEquip.category === 'Другое' && (
                                    <input type="text" required placeholder="Напишите свою категорию..." value={customCategory} onChange={e => setCustomCategory(e.target.value)} className="mt-3 w-full p-3 border dark:border-gray-600 bg-gray-50 dark:bg-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-blue-500" />
                                )}
                            </div>
                            <button type="submit" className="w-full bg-blue-600 hover:bg-blue-700 text-white font-bold py-3.5 rounded-xl shadow-lg transition-transform transform hover:-translate-y-0.5">
                                Сохранить в базу
                            </button>
                        </form>
                    </div>
                    ...
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 max-w-2xl">
                        ...
                        <form onSubmit={handleAddBulk}>
                            <textarea
                                ...
                                className="w-full p-4 border dark:border-gray-600 bg-gray-50 dark:bg-gray-700 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 font-mono text-sm mb-4 leading-relaxed whitespace-pre-wrap resize-y"
                            />
                            <button type="submit" className="w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3.5 rounded-xl shadow-lg transition-transform transform hover:-translate-y-0.5">
                            ...
                            <div className="p-6 space-y-6">
                                <form onSubmit={handleUpdateEquip} className="space-y-4">
                                    ...
                                    <div className="bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-800 p-4 rounded-xl">
                                        ...
                                        {selectedEquip.tg_id ? (
                                            <div className="flex items-center justify-between bg-white dark:bg-gray-800 p-3 rounded-lg border border-blue-200 dark:border-blue-700/50">
            ...
            {inviteInfo && (
                <div className="fixed inset-0 z-[110] bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm">
                    <div className="bg-white dark:bg-gray-800 p-6 rounded-3xl shadow-2xl w-full max-w-md">
                        <h3 className="text-xl font-bold mb-4 text-center dark:text-white">Приглашение для водителя</h3>
                        <p className="text-sm text-gray-500 dark:text-gray-400 text-center mb-6">Отправьте эту ссылку водителю. После перехода по ней, он навсегда привяжется к этой технике.</p>
                        <div className="space-y-4 mb-6">
                            <div>
                                <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1 uppercase tracking-wide">Ссылка для Telegram:</label>
                                <button onClick={() => copyToClipboard(inviteInfo.tg_bot_link, 'tg')} className="w-full text-left px-4 py-3.5 border dark:border-gray-600 rounded-xl text-sm bg-gray-50 dark:bg-gray-700 font-medium hover:bg-gray-100 transition shadow-sm text-blue-600 dark:text-blue-400">
