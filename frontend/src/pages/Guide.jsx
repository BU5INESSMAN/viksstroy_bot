    ...
    const sections = [
        {
            ...
            content: (
                <>
                    ...
                    <p><b>Навигация:</b> На мобильных устройствах используйте нижнее меню. На десктопе — боковое меню с раскрывающимися подменю, счётчиками заявок и быстрым доступом к разделам. Боковое меню можно свернуть кнопкой за его краем.</p>
        ...
        },
        {
            ...
            keywords: 'бригада команда создание приглашение join код ссылка участники удаление освобождение частичное использование свободные занятые бригадир',
            content: (
                <>
                    ...
                    <p><b>Частичное использование бригад:</b></p>
                    <ul className="list-disc ml-5 space-y-1">
                        <li>При создании заявки можно выбрать только часть бригады. Оставшиеся свободные участники доступны для других заявок на ту же дату.</li>
                        <li>Частично занятые бригады отмечены жёлтой пометкой <b>«(част.)»</b> в селекторе.</li>
                        <li>При выборе такой бригады занятые участники отображаются серым с номером заявки, в которой они уже участвуют. Свободные участники доступны для выбора, есть кнопка <b>«Выбрать всех свободных»</b>.</li>
                        <li>Если выбрана часть бригады <b>без бригадира</b>, появится предупреждение: СМР по этой группе сможет заполнить только прораб.</li>
                    </ul>
                </>
            ),
        },
        {
            ...
            keywords: 'техника добавление категория тайм-слот бронирование освобождение госномер водитель по умолчанию',
            content: (
                <>
                    <p><b>Добавление:</b> Страница "Ресурсы" → "Автопарк" → "Добавить". Укажите название, категорию, госномер. С v2.6 поле ФИО водителя удалено из формы — водитель назначается отдельно (см. ниже).</p>
                    <p><b>Массовое добавление:</b> Кнопка "Массовая загрузка". Формат с v2.6: <code className="px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-pink-600 dark:text-pink-400 font-mono text-xs">название | категория | госномер</code> (одна строка на единицу техники). Старый формат с колонкой ФИО будет отклонён с понятной ошибкой.</p>
                    <p><b>Категории и иконки:</b> Техника группируется по категориям (Самосвал, Кран, Экскаватор и т.д.). В настройках категорий можно выбрать иконку из набора Tabler Icons. Иконки отображаются в канбане и селекторах.</p>
                    <p><b>Водитель по умолчанию (v2.6):</b> На карточке каждой единицы техники есть строка «⭐ Драйвер по умолчанию». Кнопка «Изменить» (только офис: модератор+) открывает picker с водителями подходящей категории. Этот водитель будет первым в списке выбора при создании заявки, но <b>не назначается автоматически</b> — прораб подтверждает явно.</p>
                    <p><b>Тайм-слоты:</b> При создании заявки указывается время работы техники (с 08:00 до 18:00). Система проверяет пересечения и по технике, и по водителю.</p>
                    <p><b>Освобождение:</b> Прораб может освободить занятую технику до конца смены через карточку заявки.</p>
                </>
            ),
        },
        {
            id: 'drivers', icon: Users, iconColor: 'bg-cyan-100 dark:bg-cyan-900/30 text-cyan-600 dark:text-cyan-400',
            title: '6a. Водители (v2.6)', roleHint: 'Все роли',
            keywords: 'водители категории code invite default равенство занятость availability picker overlap',
            content: (
                <>
                    <p><b>Модель v2.6:</b> Водитель — отдельный пользователь системы (отдельная запись в users, как прораб или модератор). У каждого водителя:</p>
                    <ul className="list-disc pl-5 space-y-1 my-2">
                        <li><b>Личный код входа</b> (<code className="px-1.5 py-0.5 rounded bg-gray-100 dark:bg-gray-700 text-pink-600 dark:text-pink-400 font-mono text-xs">users.invite_code</code>) формата <code className="font-mono text-xs">XXXX-XXXX-XXXX</code> — водитель входит по нему, привязка к технике не нужна.</li>
                        <li><b>Категории работ</b> (экскаваторы, самосвалы, краны…) — водитель может работать с несколькими, выбирает офис.</li>
                        <li><b>История работы на технике</b> — учитывается в сортировке picker'а (недавнее и популярное — выше).</li>
                    </ul>
                    <p><b>Где назначается водитель:</b></p>
                    <ul className="list-disc pl-5 space-y-1 my-2">
                        <li><b>На странице «Техника»</b> — офис задаёт «водителя по умолчанию» для каждой единицы. Этот водитель появится первым в picker'е, но не назначается автоматически.</li>
                        <li><b>При создании/редактировании заявки</b> — прораб явно выбирает водителя для каждой единицы техники. Сначала показываются водители подходящей категории, по запросу — все остальные.</li>
                    </ul>
                    <p><b>Занятость:</b> Один водитель не может быть назначен на две разные единицы техники в пересекающиеся слоты времени на одну дату. В picker'е такие водители показаны серым с пометкой <span className="inline-flex items-center gap-1 text-[10px] font-extrabold px-1.5 py-0.5 rounded bg-red-100 text-red-700">⚠ ЗАНЯТ</span> и tooltip'ом, где они уже заняты. Защита есть и на сервере — даже если кто-то обойдёт UI, заявка с конфликтом будет отклонена.</p>
                    <p><b>Добавление водителя:</b> Создаётся одним из двух способов: офис на странице «Ресурсы» → «Водители» либо прораб inline в picker'е заявки (кнопка «+ Новый водитель»: ФИО + категории, без лишних полей). В обоих случаях генерируется личный код входа, который можно отдать водителю.</p>
                    <p><b>Старые ссылки:</b> Сохранённые ссылки вида <code className="font-mono text-xs">.../equip-invite/&lt;code&gt;</code> продолжают работать: открываются автоматически как сессия водителя «по умолчанию» этой техники (если он назначен). Старые QR-коды и закладки в браузерах не ломаются.</p>
                </>
            ),
        },
        {
        ...
        },
        {
            ...
            keywords: 'смр кп выполненные работы отчёт заполнение часы работы мастер wizard бригадир прораб проверка одобрение excel экспорт справочник должники напоминания группировка объект дата объединение слияние общие по бригадам',
            content: (
                <>
                    <p><b>Заполнение СМР — 3 шага:</b></p>
                    <ol className="list-decimal ml-5 space-y-1.5">
                        <li><b>Часы</b> — укажите отработанное время для каждой бригады. Значение на уровне бригады подставится всем сотрудникам; можно развернуть бригаду и задать часы конкретному человеку. Сотрудники в отпуске/на больничном отмечены бейджем.</li>
                        <li><b>Работы</b> — укажите объёмы выполненных работ из плана объекта. Единицы измерения (шт, м, м², т и др.) отображаются рядом с каждой позицией. Переключатель <b>«Общие / По бригадам»</b> позволяет указать общий объём на заявку или разнести его по конкретным бригадам с автоматическим итогом. Дополнительные работы выбираются из каталога КП через раскрываемый список с поиском по категориям.</li>
                        <li><b>Просмотр</b> — проверьте итоговые данные. Кнопка <i>«Редактировать»</i> возвращает к шагу 1 с сохранением введённого.</li>
                    </ol>
                    <p><b>Роли:</b></p>
                    <ul className="list-disc ml-5 space-y-1">
                        <li><b>Бригадир</b> заполняет только свою бригаду. После отправки заявка уходит на вкладку «На проверку» к прорабу.</li>
                        <li><b>Прораб</b> заполняет за все бригады. После отправки заявка сразу уходит в «Готовые».</li>
                        <li>Если несколько бригадиров работают над одной заявкой, каждый следующий видит данные другого с предупреждением.</li>
                    </ul>
                    <p><b>Объединение заявок:</b></p>
                    <ul className="list-disc ml-5 space-y-1">
                        <li>На вкладке «К заполнению» можно объединить несколько заявок одного объекта и даты — отметьте чекбоксами и нажмите «Объединить».</li>
                        <li>Объединённые заявки заполняются как одна, с бригадами и техникой из всех заявок.</li>
                        <li>Объединение можно отменить кнопкой «Разъединить» до отправки отчёта.</li>
                    </ul>
                    <p><b>Страница «СМР»:</b> заявки сгруппированы по объекту, внутри объекта — по датам. На вкладке «Готовые» рядом с каждой заявкой — кнопка <b>«Скачать»</b>. Имя файла отчёта: <code className="bg-gray-100 dark:bg-gray-700 px-1.5 py-0.5 rounded text-xs">Объект_Дата.xlsx</code>.</p>
                    <p><b>Excel-отчёт:</b> листы «Часы», «Работы» и «Доп. работы» (если есть). Столбец «Заполнил» показывает ФИО и роль автора каждой строки. Цен и зарплат в отчёте нет.</p>
                    <p><b>Справочник КП:</b> импортируется из xlsx администратором (только superadmin). Единицы измерения берутся из колонки G.</p>
                    <p><b>Должники:</b> незаполненные СМР висят в виджете на главной до заполнения. Модератор может нажать <i>«Напомнить»</i> — прорабу придёт уведомление.</p>
                    <Tip>Заполнение СМР доступно без ограничения по времени — пока отчёт не отправлен.</Tip>
                </>
            ),
        },
        {
        ...
        {
            id: 'review', icon: CheckCircle, iconColor: 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-600 dark:text-emerald-400',
            title: '25. Проверка заявок', roleHint: 'Модератор и выше',
            keywords: 'проверка модерация одобрить отклонить редактировать статус заявка автор канбан просмотр действия',
            content: (
                <>
                    <p>Все действия с заявками выполняются на странице <b>«Проверка»</b>:</p>
                    <ul className="list-disc ml-5 space-y-1">
                        <li><b>Одобрить</b> или <b>Отклонить</b> заявку (с указанием причины отклонения).</li>
                        <li><b>Редактировать заявку</b> — доступно модератору и выше, а также автору заявки.</li>
                        <li><b>Изменить статус</b> заявки — доступно модератору и выше (например, вернуть «В работе» в «На модерации»).</li>
                    </ul>
                    <p><b>Просмотр заявки в канбане</b> — только чтение, без кнопок действий. Чтобы выполнить любое действие, перейдите на страницу «Проверка».</p>
                </>
            ),
        },
        {
            id: 'object-stats', icon: ClipboardList, iconColor: 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400',
            title: '26. Статистика объекта', roleHint: 'Прораб и выше',
            keywords: 'статистика объект прогресс план факт доп работы хронология отчёты смр',
            content: (
                <>
                    <p>В статистике объекта (кнопка «Статистика» на карточке) отображаются данные из заполненных отчётов СМР:</p>
                    <ul className="list-disc ml-5 space-y-1">
                        <li><b>Общий прогресс:</b> план vs факт по каждой работе с единицами измерения.</li>
                        <li><b>Выполненные доп. работы:</b> отдельная секция с работами, добавленными вне плана.</li>
                        <li><b>Хронология выполнения:</b> все заполненные отчёты СМР, сгруппированные по датам, с указанием бригад и автора.</li>
                    </ul>
                </>
            ),
        },
