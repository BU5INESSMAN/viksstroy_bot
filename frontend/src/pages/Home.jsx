...
import ViewAppModal from '../features/applications/components/ViewAppModal'; // <-- НОВЫЙ ИМПОРТ
...
export default function Home() {
    ...
    // Новое состояние для модалки ПРОСМОТРА
    const [viewApp, setViewApp] = useState(null);

    const [appForm, setAppForm] = useState({
        id: null, date_target: smartDates[1].value, object_id: '',
        foreman_id: role === 'foreman' ? Number(tgId) : '',
        workers: [], equipment: [], status: 'waiting', plan_text: '', isViewOnly: false
    });
    ...
    // ОБНОВЛЕННАЯ ФУНКЦИЯ ДЛЯ КАНБАНА
    const handleKanbanAppClick = (app) => {
        // Просто открываем новую модалку просмотра
        setViewApp(app);
    };
    ...
    return (
        <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6 pb-24">
            ...
            {/* Модалка для Создания/Редактирования (вызывается по центральной кнопке) */}
            {isGlobalCreateAppOpen && (
                <CreateAppModal
                    appForm={appForm}
            ...
            {/* НОВАЯ МОДАЛКА ПРОСМОТРА ИЗ КАНБАНА */}
            <ViewAppModal
                app={viewApp}
