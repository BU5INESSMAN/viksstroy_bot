...
export default function ViewAppModal({ app, onClose, data }) {
    // v2.4.11: ViewAppModal is a pure read-only viewer. All actions
    // (edit, approve/reject, status transitions) live on the Review page.
    if (!app) return null;
                                            ...
                                            <div key={eq.id ?? i} className="flex items-center justify-between bg-white dark:bg-gray-800 p-3.5 rounded-xl border border-gray-100 dark:border-gray-700 gap-3">
                                                <div className="flex items-center gap-3 min-w-0">
                                                    {eq.is_freed
                                                        ...
                                                        : <div className="w-5 h-5 rounded-full bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center flex-shrink-0"><EqIcon className="w-3 h-3 text-emerald-600 dark:text-emerald-400" stroke={2} /></div>
                                                    }
