...
export default function useAppForm({
    ...
    const emptyForm = () => ({
        ...
        members_data: [],
        equipment: [],
        // v2.6: { [equipment_id]: { user_id, fio } | null }
        driverAssignments: {},
        comment: '',
        isViewOnly: false,
        foreman_id: null,
    ...
    const toggleEquipmentSelection = (equip) => {
        ...
        setAppForm(prev => {
            ...
            if (exists) {
                // Cascade: drop any driver assignment for the removed equipment.
    ...
    const setDriverForEquipment = (equipmentId, driver) => {
        ...
        const fio = driver?.fio
            || [driver?.last_name, driver?.first_name, driver?.middle_name].filter(Boolean).join(' ').trim();
        setAppForm(prev => ({
            ...prev,
            driverAssignments: {
    ...
    const checkTeamStatus = (team_id) => {
        ...
        if (totalMembers > 0 && pickedAcrossApps.size >= totalMembers) {
            return {
                state: 'busy',
                message: 'Все рабочие этой бригады уже заняты в других заявках.',
            };
        }
    ...
    const _doSubmit = async () => {
        ...
        try {
            ...
            const driversPayload = (appForm.equipment || [])
                .map((eq) => {
                    const drv = (appForm.driverAssignments || {})[eq.id];
