...
/**
 * DriverEditModal — create or edit a driver.
 * `mode`: 'create' | 'edit'
 */
export default function DriverEditModal({
    ...
    // Default equipment is only valid if its category is selected.
    ...
    const handleSave = async () => {
        ...
        try {
            const body = {
                ...
                category_names: selectedCats,
                default_equipment_id: defaultEqId ? Number(defaultEqId) : null,
            };
            let res;
            if (mode === 'edit' && initial) {
                res = await axios.patch(`/api/drivers/${initial.user_id}`, body);
            } else {
                res = await axios.post('/api/drivers', body);
            }
